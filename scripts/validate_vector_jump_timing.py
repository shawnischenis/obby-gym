...
def run_group(
    ...
    for step in range(30):
        ...
        for index, jump_step in enumerate(jump_steps):
            ...
            if jump:
                takeoff_distances[index] = float(
                    np.linalg.norm(
                        observations[index, 6:9] * np.asarray([64.0, 32.0, 64.0])
                    )
                )
            actions.append(
                {
        ...
        if not any(active):
            break
    return [
        {"jump_step": step, "distance": distance, "gap": gap, "outcome": outcome}
        for step, distance, gap, outcome in zip(
            jump_steps, takeoff_distances, gaps, outcomes, strict=True
        )
    ]
